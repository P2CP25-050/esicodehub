# Sprint 5 — Plagiarism Detection & Q&A Forum (Updated)
...
EXTENSION_LANGUAGE_MAP = {
    ...
    'html': 'javascript',  # Skip — MOSS doesn't support HTML meaningfully
    'css':  'javascript',  # Skip — same
}
```
...
**Threaded replies:** Answers are recursive. The `Answer` model has a nullable `parent` ForeignKey pointing to another `Answer`. This creates a tree. However to keep things manageable and consistent with Reddit's model, **only one reply per user per answer is allowed** (`unique_together = ['parent', 'author']`). A user can reply to an answer once, and someone can reply to that reply once, and so on — forming chains, not trees. This is enforced at the model and API level.
...
class AnswerSerializer(serializers.ModelSerializer):
    ...
    def get_replies(self, obj):
        # Only fetch one level of replies — client can request deeper nesting separately
...
- Author filter — a text input for filtering by author email or name, maps to `author` param
